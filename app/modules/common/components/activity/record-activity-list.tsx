...
/**
 * Generic activity list component for any record type.
 * No permissions required - all authenticated users can view activity history.
 */
export default async function RecordActivityList({
  recordId,
  recordType,
  limit = 10,
}: RecordActivityListProps) {
  const activities = await getRecentActivitiesAction(limit, {
    recordId,
    recordType,
  });
    ...
    return (
      <div className="space-y-3">
        ...
        <p className="text-xs text-muted-foreground">
          No activity yet. Changes to this {recordType.toLowerCase()} will appear here.
        </p>
      </div>
    );
  }
